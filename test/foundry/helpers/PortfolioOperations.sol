...
contract PortfolioOperations is Permit2Helper {
    ...
    function _mintTokensForUser(address _token, address _user, uint256 _amount) internal {
    ...
    function depositChecks(
        ...
        uint256[] memory tokenAmountsAfter = getTokenBalances(portfolioTokens, _depositor);
        ...
        for (uint256 i; i < portfolioTokensLength; i++) {
            ...
            if (_depositor == owner) {
                ...
                console.log("total amount deposited owner", totalAmountDepositedOwner[i]);
            } else if (_depositor == nonOwner) {
                totalAmountDepositedNonOwner[i] += depositAmount;
                console.log("total amount deposited non-owner", totalAmountDepositedNonOwner[i]);
            }
        }
    }
    ...
    function _depositFor(
        ...
        for (uint256 i; i < depositAmountsLength; i++) {
            IERC20(portfolioTokens[i]).approve(address(portfolio), _depositAmounts[i]);
        }

        uint256[] memory tokenAmountsBefore = getTokenBalances(portfolioTokens, _depositor);

        portfolio.multiTokenDepositFor(_depositInBehalfOf, _depositAmounts, minMintAmount);
    ...
    function _deposit(address _depositor, uint256 _privateKey, uint256[] memory _depositAmounts, uint256 minMintAmount)
        internal
    {
        if (_depositor == owner) {
            console.log("--- DEPOSIT OWNER ---");
        ...
        for (uint256 i; i < portfolioTokensLength; i++) {
            ...
            (,, uint48 nonce) = permit2.allowance(_depositor, portfolioToken, address(portfolio));

            nonces[i] = nonce;
        }

        IAllowanceTransfer.PermitBatch memory permit = defaultERC20PermitBatchAllowance(
            portfolioTokens, _depositAmounts, uint48(block.timestamp + 100), nonces, address(portfolio)
        );
        bytes memory sig = getPermitBatchSignature(permit, _privateKey, DOMAIN_SEPARATOR);
        ...
        depositChecks(
            _depositor, portfolioTokens, portfolioTokensLength, tokenAmountsBefore, userBalanceBefore, supplyBefore
        );
    }
    ...
    function _withdrawFor(
        ...
        uint256[] memory tokenAmountsBefore = getTokenBalances(portfolioTokens, _tokenReceiver);
        ...
        portfolio.multiTokenWithdrawalFor(_withdrawInBehalfOf, _tokenReceiver, _withdrawAmount, repayData);
        vm.stopPrank();

        uint256[] memory tokenAmountsAfter = getTokenBalances(portfolioTokens, _tokenReceiver);
        ...
        for (uint256 i; i < portfolioTokensLength; i++) {
            assertTrue(
                ...
                string(
                    abi.encodePacked(
                        "Token balance of the following token does not increase after withdrawal: ", portfolioTokens[i]
                    )
                )
            );

            console.log("withdrawal amount", portfolioTokens[i], tokenAmountsAfter[i] - tokenAmountsBefore[i]);
        }
    }
    ...
    function _withdraw(
        ...
        uint256[] memory tokenAmountsBefore = getTokenBalances(portfolioTokens, _withdrawer);
        ...
        uint256[] memory tokenAmountsAfter = getTokenBalances(portfolioTokens, _withdrawer);
    ...
    function getTokenBalances(address[] memory portfolioTokens, address _holder)
        internal
        view
        returns (uint256[] memory tokenAmountsBefore)
    {
        uint256 portfolioTokensLength = portfolioTokens.length;
