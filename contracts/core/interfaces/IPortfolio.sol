...
interface IPortfolio {
    ...
    function init(FunctionParameters.PortfolioInitData calldata initData) external;
    ...
    // q repay data is used in this function but no in the docs multiTokenWithdrawalFor(address _withdrawFor, address _tokenReceiver, uint256 _portfolioTokenAmount) external
    function multiTokenWithdrawalFor(
        address _withdrawFor,
        address _tokenReceiver,
        uint256 _portfolioTokenAmount,
        // q what if the borrower didn't take the loan?
        FunctionParameters.withdrawRepayParams calldata repayData
    ) external;
    ...
    function getTokenBalancesOf(address[] memory, address) external view returns (uint256[] memory);

    function getVaultValueInUSD(IPriceOracle, address[] memory, uint256, address) external view returns (uint256);

    function _calculateMintAmount(uint256, uint256) external returns (uint256);

    function vaultInteraction(address _target, bytes memory _claimCalldata) external;
}
