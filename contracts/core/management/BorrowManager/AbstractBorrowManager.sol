 ...
 */
abstract contract AbstractBorrowManager is OwnableUpgradeable, UUPSUpgradeable, AccessModifiers, IBorrowManager {
     ...
     */
    function init(address vault, address protocolConfig, address portfolio, address accessController)
        external
        override
        initializer
    {
    ...
    function repayBorrow(
        ...
        beforeRepayVerification(
            repayData._factory, repayData._solverHandler, repayData._swapHandler, repayData._bufferUnit
        );
        ...
        for (uint256 j; j < controllersLength; j++) {
            ...
            IAssetHandler assetHandler = IAssetHandler(_protocolConfig.assetHandlers(_controller));

            address[] memory borrowedTokens = assetHandler.getBorrowedTokens(_vault, _controller); // Get all borrowed tokens for the vault under the controller
     ...
     */
    function repayVault(address _controller, FunctionParameters.RepayParams calldata repayData)
        external
        ...
        returns (bool)
    {
        ...
        bytes memory data =
            abi.encodeWithSelector(IAssetHandler.executeVaultFlashLoan.selector, address(this), repayData);
        ...
        TransferHelper.safeTransfer(
            repayData._flashLoanToken, _vault, IERC20Upgradeable(repayData._flashLoanToken).balanceOf(address(this))
        );
