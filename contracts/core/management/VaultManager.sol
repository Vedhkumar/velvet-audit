    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function __VaultManager_init(address protocolConfig, address borrowManager) internal onlyInitializing {
    ...
    function multiTokenDeposit(
        ...
        _multiTokenDepositWithPermit(msg.sender, depositAmounts, _minMintAmount, _permit, _signature);
    }
     ...
     *      The nonReentrant modifier is used to prevent reentrancy attacks.
     */
    function multiTokenDepositFor(address _depositFor, uint256[] calldata depositAmounts, uint256 _minMintAmount)
        external
        virtual
        nonReentrant
    {
     ...
     */
    // @audit i no param for the repayData
    function multiTokenWithdrawalFor(
        ...
        _multiTokenWithdrawal(_withdrawFor, _tokenReceiver, _portfolioTokenAmount, _emptyArray, repayData);
    }
    ...
    function multiTokenWithdrawal(
        ...
        address[] memory _emptyArray;
        _multiTokenWithdrawal(msg.sender, msg.sender, _portfolioTokenAmount, _emptyArray, repayData);
    }
    ...
    function emergencyWithdrawal(
        ...
        _multiTokenWithdrawal(msg.sender, msg.sender, _portfolioTokenAmount, _exemptionTokens, repayData);
    }
    ...
    function emergencyWithdrawalFor(
        ...
        _spendAllowance(_withdrawFor, msg.sender, _portfolioTokenAmount);
        _multiTokenWithdrawal(_withdrawFor, _tokenReceiver, _portfolioTokenAmount, _exemptionTokens, repayData);
    }
     ...
     */
    function pullFromVault(address _token, uint256 _amount, address _to) external onlyRebalancerContract {
     ...
     * @param _to The destination address for the tokens.
     */
    function _pullFromVault(address _token, uint256 _amount, address _to) internal {
        // Prepare the data for ERC20 token transfer
        bytes memory inputData = abi.encodeWithSelector(IERC20Upgradeable.transfer.selector, _to, _amount);

        // Execute the transfer through the safe module and check for success
        (, bytes memory data) = IVelvetSafeModule(safeModule).executeWallet(_token, inputData);
     ...
     */
    function vaultInteraction(address _target, bytes memory _claimCalldata) external onlyRebalancerContract {
     ...
     */
    function _vaultInteraction(address _target, bytes memory _claimCalldata) internal {
        // Execute the transfer through the safe module and check for success
        (bool success,) = IVelvetSafeModule(safeModule).executeWallet(_target, _claimCalldata);
    ...
    function _multiTokenDepositWithPermit(
        ...
        uint256 _depositRatio = _multiTokenTransferWithPermit(depositAmounts, _permit, _signature, msg.sender);
     ...
     * @param _minMintAmount The minimum amount of portfolio tokens the user expects to receive for their deposit, protecting against slippage.
     */
    function _multiTokenDeposit(address _depositFor, uint256[] calldata depositAmounts, uint256 _minMintAmount)
        internal
        virtual
    {
        // Verify that the user is allowed to deposit and that the system is not paused.
     ...
     */
    function _depositAndMint(address _depositFor, uint256 _minMintAmount, uint256 _depositRatio) internal {
        ...
        emit Deposited(address(this), _depositFor, tokenAmount, balanceOf(_depositFor));
    }
    ...
    function _multiTokenWithdrawal(
        ...
        _performWithdrawalChecks(_withdrawFor, _portfolioTokenAmount, portfolioTokenLength, _exemptionTokens);
        // t the _expemtionTokens array to be empty for func multiTokenWithdrawal but there is check between portfolioTokens and _exemptionTokens in ChecksAndValidations.sol
        ...
        _borrowManager.repayBorrow(_portfolioTokenAmount, totalSupplyPortfolio, repayData);
        ...
        uint256[] memory userWithdrawalAmounts = _processTokenWithdrawals(
            _tokenReceiver, _portfolioTokenAmount, totalSupplyPortfolio, portfolioTokens, _exemptionTokens
        );
    ...
    function _performWithdrawalChecks(
        ...
        _beforeWithdrawCheck(
            _withdrawFor, IPortfolio(address(this)), _portfolioTokenAmount, portfolioTokenLength, _exemptionTokens
        );
    ...
    function _processTokenWithdrawals(
        ...
        uint256 portfolioTokenLength = portfolioTokens.length;
        uint256[] memory userWithdrawalAmounts = new uint256[](portfolioTokenLength);
        uint256 exemptionIndex = 0;

        // Get controllers data for the vault
        TokenBalanceLibrary.ControllerData[] memory controllersData =
            TokenBalanceLibrary.getControllersData(vault, portfolioTokens, _protocolConfig);
    ...
    function _processTokenWithdrawal(
        ...
        uint256 tokenBalance =
            TokenBalanceLibrary._getAdjustedTokenBalance(_token, vault, _protocolConfig, controllersData);
        tokenBalance = (tokenBalance * _portfolioTokenAmount) / totalSupplyPortfolio;

        // Prepare the data for ERC20 token transfer
        bytes memory inputData =
            abi.encodeWithSelector(IERC20Upgradeable.transfer.selector, _tokenReceiver, tokenBalance);
     ...
     */
    function _transferToVaultWithPermit(address _from, address _token, uint256 _depositAmount) internal {
        permit2.transferFrom(_from, vault, MathUtils.safe160(_depositAmount), _token);
    }
     ...
     * @param _depositAmount Amount of the token to be transferred.
     */
    function _transferToVault(address _from, address _token, uint256 _depositAmount) internal {
    ...
    function _multiTokenTransferWithPermit(
        ...
        try permit2.permit(msg.sender, _permit, _signature) {
            ...
            for (uint256 i; i < depositAmountsLength; i++) {
                if (IERC20Upgradeable(portfolioTokens[i]).allowance(msg.sender, address(this)) < depositAmounts[i]) {
        ...
        return _handleTokenTransfer(
            _depositFor, amountLength, depositAmounts, portfolioTokens, tokenBalancesBefore, true, controllersData
        );
    }
     ...
     * @return The minimum deposit ratio after deposits.
     */
    function _multiTokenTransfer(address _from, uint256[] calldata depositAmounts) internal returns (uint256) {
        // Validate deposit amounts and get initial token balances
        (
            uint256 amountLength,
            address[] memory portfolioTokens,
        ...
        // Handles the token transfer and minRatio calculations
        return _handleTokenTransfer(
            _from, amountLength, depositAmounts, portfolioTokens, tokenBalancesBefore, false, controllersData
        );
    }
     ...
     */
    function _validateAndGetBalances(uint256[] calldata depositAmounts)
        internal
        view
        returns (uint256, address[] memory, uint256[] memory, TokenBalanceLibrary.ControllerData[] memory)
    {
        ...
        (uint256[] memory tokenBalancesBefore, TokenBalanceLibrary.ControllerData[] memory controllersData) =
            TokenBalanceLibrary.getTokenBalancesOf(portfolioTokens, vault, _protocolConfig);

        return (amountLength, portfolioTokens, tokenBalancesBefore, controllersData);
    }
    ...
    function _handleTokenTransfer(
        ...
        if (totalSupply() == 0) {
            return _handleEmptyVaultTransfer(
                _from, amountLength, depositAmounts, portfolioTokens, tokenBalancesBefore, usePermit
            );
        }

        uint256 _minRatio = _calculateMinRatio(amountLength, depositAmounts, tokenBalancesBefore);
        return _executeTransfers(
            _from, amountLength, portfolioTokens, tokenBalancesBefore, _minRatio, usePermit, controllersData
        );
    }
    ...
    function _handleEmptyVaultTransfer(
        ...
        for (uint256 i; i < amountLength; i++) {
            ...
            if (TokenBalanceLibrary._getTokenBalanceOf(token, vault) <= tokenBalancesBefore[i]) {
                revert ErrorLibrary.TransferFailed();
            }
    ...
    function _calculateMinRatio(
        ...
        for (uint256 i = 0; i < amountLength; i++) {
            uint256 _currentRatio = _getDepositToVaultBalanceRatio(depositAmounts[i], tokenBalancesBefore[i]);
    ...
    function _executeTransfers(
        ...
        for (uint256 i; i < amountLength; i++) {
            ...
            uint256 transferAmount = (_minRatio * tokenBalanceBefore) / ONE_ETH_IN_WEI;
            depositedAmounts[i] = transferAmount;

            _transferToken(_from, token, transferAmount, usePermit);

            uint256 tokenBalanceAfter =
                TokenBalanceLibrary._getAdjustedTokenBalance(token, vault, _protocolConfig, controllersData);
            uint256 currentRatio =
                _getDepositToVaultBalanceRatio(tokenBalanceAfter - tokenBalanceBefore, tokenBalanceAfter);
            _minRatioAfterTransfer = MathUtils._min(currentRatio, _minRatioAfterTransfer);
        }
     ...
     */
    function _transferToken(address _from, address token, uint256 amount, bool usePermit) private {
        if (usePermit) {
