...
contract VenusAssetHandler is IAssetHandler, ExponentialNoError {
    address public constant vBNB_Address = 0xA07c5b74C9B40447a954e1466938b865b6BBea36;
     ...
     */
    struct AccountVars {
        uint256 totalCollateral; // Total collateral amount
        uint256 totalDebt; // Total debt amount
        uint256 availableBorrows; // Available borrowing capacity
        ...
        uint256 lendCount; // Number of assets being lent
        uint256 borrowCount; // Number of assets being borrowed
    }
    ...
    struct AccountLiquidityLocalVars {
        uint256 totalCollateral; // Total collateral amount
        uint256 sumCollateral; // Summed collateral amount
        uint256 sumBorrowPlusEffects; // Summed borrow amount plus any effects
        uint256 vTokenBalance; // Balance of vTokens (collateralized assets)
        uint256 borrowBalance; // Balance of borrowed assets
        uint256 exchangeRateMantissa; // Exchange rate of vTokens
        uint256 oraclePriceMantissa; // Price of the asset from the oracle
    ...
    struct TokenInfo {
        ...
        uint256 count; // Number of tokens
    }
     ...
     */
    function enterMarket(address[] memory assets) external pure returns (bytes memory data) {
     ...
     */
    function borrow(address, address, address, uint256 borrowAmount) external pure returns (bytes memory data) {
        data = abi.encodeWithSelector(
     ...
     */
    function repay(address, address, uint256 borrowAmount) public pure returns (bytes memory data) {
        data = abi.encodeWithSelector(
     ...
     */
    function approve(address _toApprove, uint256 _amountToApprove) public pure returns (bytes memory data) {
        data = abi.encodeCall(
    ...
    function withdraw(address, address, uint256 amount) public pure returns (bytes memory data) {
    ...
    function swapTokens(
        ...
        bytes memory encodedParams = abi.encode(path, recipient, block.timestamp + 15, amountIn, minAmountOut);

        data = abi.encodeWithSelector(
            bytes4(keccak256("exactInput((bytes,address,uint256,uint256,uint256))")), encodedParams
        );
    }
     ...
     */
    function getAllProtocolAssets(address account, address comptroller, address[] memory)
        public
        view
        returns (address[] memory lendTokens, address[] memory borrowTokens)
    {
        address[] memory assets = IVenusComptroller(comptroller).getAssetsIn(account); // Get the assets in the account
        uint256 assetsCount = assets.length; // Get the number of assets
        ...
        for (uint256 i = 0; i < assetsCount;) {
            ...
            (, uint256 vTokenBalance, uint256 borrowBalance,) = asset.getAccountSnapshot(account); // Get the account snapshot
     ...
     */
    function getUserAccountData(address user, address comptroller, address[] memory)
        public
        view
        returns (
        ...
        )
    {
        ...
        accountData.healthFactor = accountData.totalDebt == 0
            ? type(uint256).max // Set health factor to max if no debt
     ...
     * @return borrowTokens An array of addresses representing borrowed assets.
     */
    function getAccountPosition(address comptroller, address account)
        internal
        view
        returns (
            uint256 totalCollateral,
        ...
        )
    {
        ...
        uint256 assetsCount = assets.length; // Number of assets
        ...
        (vars, lendInfo, borrowInfo) =
            processAssets(comptroller, account, assets, assetsCount, vars, lendInfo, borrowInfo);

        // Handle VAIController logic separately
        vars.sumBorrowPlusEffects = handleVAIController(comptroller, account, vars.sumBorrowPlusEffects);
    ...
    function processAssets(
        ...
        TokenInfo memory lendInfo,
        TokenInfo memory borrowInfo
    ) internal view returns (AccountLiquidityLocalVars memory, TokenInfo memory, TokenInfo memory) {
        ...
        for (uint256 i = 0; i < assetsCount;) {
            ...
            (lendInfo.count, borrowInfo.count) = processTokenBalances(
                asset, comptroller, vars, lendInfo.tokens, lendInfo.count, borrowInfo.tokens, borrowInfo.count
            );
            unchecked {
                ++i;
            }
     ...
     */
    function updateVarsWithSnapshot(IVenusPool asset, address account, AccountLiquidityLocalVars memory vars)
        internal
        view
        returns (bool shouldContinue)
    {
        (uint256 oErr, uint256 vTokenBalance, uint256 borrowBalance, uint256 exchangeRateMantissa) =
    ...
    function processTokenBalances(
        ...
        (, uint256 collateralFactorMantissa,) = IVenusComptroller(comptroller).markets(address(asset));
        vars.collateralFactor = Exp({mantissa: collateralFactorMantissa});
        vars.exchangeRate = Exp({mantissa: vars.exchangeRateMantissa});

        // Get the normalized price of the asset
        vars.oraclePriceMantissa = IVenusComptroller(comptroller).oracle().getUnderlyingPrice(address(asset));
     ...
     */
    function handleVAIController(address comptroller, address account, uint256 sumBorrowPlusEffects)
        internal
        view
        returns (uint256)
    {
        IVAIController vaiController = IVenusComptroller(comptroller).vaiController(); // Get the VAI controller from the comptroller
    ...
    function getBorrowedTokens(address user, address comptroller)
        external
        view
        returns (address[] memory borrowedTokens)
    {
        ...
        for (uint256 i = 0; i < assetsCount;) {
            ...
            (oErr, vars.vTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(user);
            if (vars.borrowBalance > 0) {
     ...
     */
    function getInvestibleBalance(address _token, address _vault, address _controller, address[] memory portfolioTokens)
        external
        view
        returns (uint256)
    {
        // Get the account data for the vault
        (FunctionParameters.AccountData memory accountData,) = getUserAccountData(_vault, _controller, portfolioTokens);

        // Calculate the unused collateral percentage
        uint256 unusedCollateralPercentage = accountData.totalCollateral == 0
            ? 10 ** 18
            : ((accountData.totalCollateral - accountData.totalDebt) * 10 ** 18) / accountData.totalCollateral;
    ...
    function processLoanTransactions(LoanProcessingParams memory params)
        internal
        view
        returns (MultiTransaction[] memory transactions, uint256 totalFlashAmount)
    {
        ...
        ProcessedTransactions memory txResult = processRepayAndWithdraw(params, swapResult.feeCount);
    ...
    function processSwaps(LoanProcessingParams memory params) internal view returns (ProcessedSwaps memory result) {
        (result.transactions, result.flashAmount, result.feeCount) =
            swapAndTransferTransactionsUsingDex(params.vault, params.executor, params.flashData);
    }
    ...
    function processRepayAndWithdraw(LoanProcessingParams memory params, uint256 feeCount)
        internal
        view
        returns (ProcessedTransactions memory result)
    {
        result.repayTx = repayTransactions(params.executor, params.vault, params.flashData);
    ...
    function combineTransactions(TransactionArrays memory txArrays) private pure returns (MultiTransaction[] memory) {
        // Combine all transactions into one array - keeping original array size calculation
        MultiTransaction[] memory transactions =
            new MultiTransaction[](txArrays.swapTx.length + txArrays.repayTx.length + txArrays.withdrawTx.length);
        uint256 count;

        // Add swap transactions to the final array
        for (uint256 i = 0; i < txArrays.swapTx.length;) {
        ...
        for (uint256 i = 0; i < txArrays.repayTx.length;) {
        ...
        for (uint256 i = 0; i < txArrays.withdrawTx.length;) {
    ...
    function loanProcessing(
        ...
        uint256 fee,
        FunctionParameters.FlashLoanData memory flashData
    ) external view returns (MultiTransaction[] memory transactions, uint256 totalFlashAmount) {
        if (flashData.isDexRepayment) {
            (transactions, totalFlashAmount) =
                loanProcessingDex(vault, executor, controller, receiver, lendTokens, totalCollateral, fee, flashData);
        } else {
            // Process swaps and transfers during the loan
            (MultiTransaction[] memory swapTransactions, uint256 flashLoanAmount) =
                swapAndTransferTransactions(vault, flashData);

            // Handle repayment transactions
            MultiTransaction[] memory repayLoanTransaction = repayTransactions(executor, vault, flashData);

            // Handle withdrawal transactions
            MultiTransaction[] memory withdrawTransaction =
                withdrawTransactions(executor, vault, controller, receiver, lendTokens, totalCollateral, fee, flashData);

            // Combine all transactions into one array
            transactions = new MultiTransaction[](
                swapTransactions.length + repayLoanTransaction.length + withdrawTransaction.length
            );
            uint256 count;

            // Add swap transactions to the final array
            uint256 swapTransactionsLength = swapTransactions.length;
            for (uint256 i = 0; i < swapTransactionsLength;) {
            ...
            for (uint256 i = 0; i < repayLoanTransactionLength;) {
            ...
            for (uint256 i = 0; i < withdrawTransactionLength;) {
    ...
    function swapAndTransferTransactionsUsingDex(
        ...
        address executor,
        FunctionParameters.FlashLoanData memory flashData
    ) internal view returns (MultiTransaction[] memory transactions, uint256 totalFlashAmount, uint256 feeCount) {
        ...
        (transactions, count, totalFlashAmount, feeCount) =
            createSwapTransactions(context, flashData, transactions, count, feeCount);
    ...
    function createSwapTransactions(
        ...
        uint256 count,
        uint256 feeCount
    ) internal view returns (MultiTransaction[] memory, uint256, uint256, uint256) {
        uint256 totalFlashAmount;

        for (uint256 i; i < flashData.debtToken.length;) {
            if (context.flashLoanToken != flashData.debtToken[i]) {
                ...
                transactions[count].txData = abi.encodeWithSelector(
                    bytes4(keccak256("transfer(address,uint256)")), context.vault, flashData.flashLoanAmount[i]
                );
                count++;
     ...
     * @return totalFlashAmount The total amount of flash loan used.
     */
    function swapAndTransferTransactions(address vault, FunctionParameters.FlashLoanData memory flashData)
        internal
        pure
        returns (MultiTransaction[] memory transactions, uint256 totalFlashAmount)
    {
        ...
        for (uint256 i; i < tokenLength;) {
            ...
            if (flashData.flashLoanToken != flashData.debtToken[i]) {
                ...
                transactions[count].txData = abi.encodeWithSelector(
                    bytes4(keccak256("multiTokenSwapAndTransfer(address,bytes)")), vault, flashData.firstSwapData[i]
                );
                count++;
            }
     ...
     * @return transactions An array of transactions to execute.
     */
    function repayTransactions(address executor, address vault, FunctionParameters.FlashLoanData memory flashData)
        internal
        pure
    ...
    function withdrawTransactionsUsingDex(
        ...
        return processWithdrawTransactions(context, lendingTokens, flashData, feeCount);
    }
    ...
    function processWithdrawTransactions(
        ...
        (count, feeCount) = processLendingTokenBatch(
            _context, lendingTokens, sellAmounts, transactions, count, feeCount, flashData.poolFees
        );
        // Resize the transactions array to remove unused entries
        uint256 unusedLength = ((tokenLength * 4) - count);
        assembly {
            mstore(transactions, sub(mload(transactions), unusedLength))
        }
    ...
    function processLendingTokenBatch(
        ...
        for (uint256 j = 0; j < lendingTokens.length;) {
            address lendingToken = lendingTokens[j]; // Using index from original logic
            address underlying = (lendingToken == vBNB_Address) ? WBNB_Address : IVenusPool(lendingToken).underlying();
            WithdrawContext memory _context = context;
            ...
            uint256 underlyingAmount = getUnderlyingAmount(lendingToken, _context.user, _sellAmount);
            ...
            transactions[count].txData = abi.encodeWithSelector(
                ...
                ISwapHandler(_context.swapHandler).swapExactTokensForTokens(
                    underlying, _context.flashloanToken, _context.receiver, underlyingAmount, 0, fee
                )
            );
            count++;
            feeCount++;
     ...
     */
    function getUnderlyingAmount(address vToken, address account, uint256 vTokenBalance)
        internal
        view
        returns (uint256 underlyingAmount)
    {
        ...
        (oErr,,, exchangeRateMantissa) = IVenusPool(vToken).getAccountSnapshot(account);
    ...
    function withdrawTransactions(
        ...
        for (uint256 j = 0; j < lendingTokensLength;) {
    ...
    function executeUserFlashLoan(
        ...
        uint256 borrowedLength = borrowedTokens.length;
        ...
        address _poolAddress = IThena(repayData._factory).poolByPair(repayData._token0, repayData._token1);

        // Prepare the flash loan data to be used in the flash loan callback
        FunctionParameters.FlashLoanData memory flashData = FunctionParameters.FlashLoanData({
    ...
    function executeVaultFlashLoan(address _receiver, FunctionParameters.RepayParams calldata repayData) external {
        ...
        IAlgebraPool(_poolAddress).flash(
            _receiver, // Recipient of the flash loan
            repayData._token0 == repayData._flashLoanToken ? repayData._flashLoanAmount[0] : 0, // Amount of token0 to flash loan
            repayData._token1 == repayData._flashLoanToken ? repayData._flashLoanAmount[0] : 0, // Amount of token1 to flash loan
            abi.encode(flashData) // Encode flash loan data to pass to the callback
        );
    }
    ...
    function getCollateralAmountToSell(
        ...
        for (uint256 i; i < _protocolToken.length;) {
            uint256 borrowBalance = IVenusPool(_protocolToken[i]).borrowBalanceStored(_user); // Get the borrow balance for the protocol token

            uint256 oraclePriceMantissa = IVenusComptroller(_controller).oracle().getUnderlyingPrice(_protocolToken[i]); // Get the oracle price for the protocol token
            ...
            sumBorrowPlusEffects = handleVAIController(_controller, _user, sumBorrowPlusEffects);
            ...
            (, uint256 percentageToRemove) = calculateDebtAndPercentage(
                _debtRepayAmount[i], feeUnit, sumBorrowPlusEffects / 10 ** 10, borrowBalance, totalCollateral
            );

            // Calculate the amounts to sell for each lending token
            amounts = calculateAmountsToSell(_user, lendTokens, percentageToRemove, bufferUnit, amounts);
            unchecked {
                ++i;
            }
        }
