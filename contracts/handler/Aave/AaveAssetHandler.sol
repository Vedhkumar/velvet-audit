...
contract AaveAssetHandler is IAssetHandler {
    address immutable DATA_PROVIDER_ADDRESS = 0x7F23D86Ee20D869112572136221e173428DD740B;

    address immutable PRICE_ORACLE_ADDRESS = 0xb56c2F0B653B2e0b10C9b928C8580Ac5Df02C7C7;
     ...
     */
    function borrow(address, address asset, address onBehalfOf, uint256 borrowAmount)
        external
        pure
     ...
     */
    function repay(address asset, address onBehalfOf, uint256 borrowAmount) public pure returns (bytes memory data) {
        data = abi.encodeWithSelector(
    ...
    function withdraw(address asset, address to, uint256 amount) public pure returns (bytes memory data) {
        data = abi.encodeWithSelector(bytes4(keccak256("withdraw(address,uint256,address)")), asset, amount, to);
    }
    ...
    function getAllProtocolAssets(
    ...
    ) public view returns (address[] memory lendTokens, address[] memory borrowTokens) {
        ...
        for (uint256 i = 0; i < portfolioTokensLength; i++) {
        ...
        for (uint256 i = 0; i < assetsCount;) {
            ...
            (,, uint256 currentVariableDebt,,,,,,) =
                IPoolDataProvider(DATA_PROVIDER_ADDRESS).getUserReserveData(assets[i], account);
            DataTypes.ReserveDataLegacy memory data = IAavePool(comptroller).getReserveData(asset);
     ...
     */
    function getUserAccountData(address user, address comptroller, address[] memory portfolioTokens)
        public
        view
        returns (
        ...
        )
    {
        ...
        (tokenAddresses.lendTokens, tokenAddresses.borrowTokens) =
            getAllProtocolAssets(user, comptroller, portfolioTokens);
    }

    function getBorrowedTokens(address account, address comptroller)
        external
        view
    ...
    function processLendingTokenBatch(
        ...
        for (uint256 j = 0; j < lendingTokens.length;) {
            address underlying = IAaveToken(lendingTokens[j]).UNDERLYING_ASSET_ADDRESS();
            ...
            transactions[count++] = MultiTransaction({
                to: _context.executor,
                txData: abi.encodeWithSelector(
                    bytes4(keccak256("vaultInteraction(address,bytes)")), underlying, approve(_context.router, _sellAmount)
                )
            });
            ...
            transactions[count++] = MultiTransaction({
                ...
                txData: abi.encodeWithSelector(
                    ...
                    ISwapHandler(_context.swapHandler).swapExactTokensForTokens(
                        underlying, _context.flashloanToken, _context.receiver, _sellAmount, 1, fee
                    )
                )
            });
            feeCount++;
    ...
    function getCollateralAmountToSell(
        ...
        for (uint256 i; i < _protocolToken.length;) {
            ...
            address _underlyingToken = IAaveToken(_protocolToken[i]).UNDERLYING_ASSET_ADDRESS();
            (,, uint256 currentVariableDebt,,,,,,) =
                IPoolDataProvider(DATA_PROVIDER_ADDRESS).getUserReserveData(_underlyingToken, _user);

            //Convert underlyingToken to 18 decimal
            uint256 borrowBalance =
                currentVariableDebt * 10 ** (18 - IERC20MetadataUpgradeable(_underlyingToken).decimals());

            //Get price for _protocolToken token and convert to 18 decimal
            uint256 _oraclePrice = IAavePriceOracle(PRICE_ORACLE_ADDRESS).getAssetPrice(_underlyingToken) * 10 ** 10;

            address user = _user;
            //Get price for borrow Balance (amount * price)
            uint256 _tokenPrice = (borrowBalance * _oraclePrice) / 10 ** 18; // Converting to 18 decimal
            ...
            amounts = calculateAmountsToSell(user, lendTokens, percentageToRemove, bufferUnit, amounts);
            unchecked {
                ++i;
            }
        }
    ...
    function executeUserFlashLoan(
        ...
        for (uint256 i; i < borrowedLength;) {
            address _underlyingToken = IAaveToken(borrowedTokens[i]).UNDERLYING_ASSET_ADDRESS();
            (,, uint256 currentVariableDebt,,,,,,) =
                IPoolDataProvider(DATA_PROVIDER_ADDRESS).getUserReserveData(_underlyingToken, _vault);
            underlying[i] = _underlyingToken; // Get the underlying asset for the borrowed token
            tokenBalance[i] = (currentVariableDebt * _portfolioTokenAmount) / _totalSupply; // Calculate the portion of the debt to repay
        ...
        IAavePool(repayData._token0).flashLoan(
            receiver, assets, amounts, interestRateModes, receiver, abi.encode(flashData), 0
        );
    }

    function executeVaultFlashLoan(address _receiver, FunctionParameters.RepayParams calldata repayData)
        external
        override
    {
        ...
        IAavePool(repayData._token0).flashLoan(
            _receiver, assets, amounts, interestRateModes, _receiver, abi.encode(flashData), 0
        );
    }
}
