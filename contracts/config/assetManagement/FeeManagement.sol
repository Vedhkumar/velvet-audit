...
abstract contract FeeManagement is AssetManagerCheck, Initializable {
    ...
    function __FeeManagement_init(
        ...
        if (
            _managementFee > protocolConfig.maxManagementFee() || _performanceFee > protocolConfig.maxPerformanceFee()
                || _entryFee > protocolConfig.maxEntryFee() || _exitFee > protocolConfig.maxExitFee()
     ...
     */
    function proposeNewManagementFee(uint256 _newManagementFee) external onlyAssetManager {
    ...
    function updateManagementFee() external onlyAssetManager {
        ...
        feeModule.chargeProtocolAndManagementFees(); // q what this funtion do?
     ...
     */
    function proposeNewPerformanceFee(uint256 _newPerformanceFee) external onlyAssetManager {
     ...
     */
    function proposeNewEntryAndExitFee(uint256 _newEntryFee, uint256 _newExitFee) external onlyAssetManager {
        if (_newEntryFee > protocolConfig.maxEntryFee() || _newExitFee > protocolConfig.maxExitFee()) {
            revert ErrorLibrary.InvalidFee();
        }
